...
if [ -d /opt/arcgis/server ]; then
    echo "Server software already installed."
else
...
AGSURL="https://$FQDN:6080/arcgis"

# use the admin api to set up the server site using the default config-store and directories locations
curl --retry 20 -X POST \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d 'username=admin&password=Esri3801&f=json' \
  "http://$FQDN:6080/arcgis/admin/createNewSite" >> /tmp/server_curl
if [ $? == 0 ]; then
    echo "ArcGIS Server configured."
else
    echo "ArcGIS Server setup failed, not reachable at $AGSURL"
    # Comment out next line to press on to next step.
    #exit 0
fi
